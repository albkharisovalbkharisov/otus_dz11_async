...
struct bulki
{
	using std::list<std::pair<bulk, std::mutex>> = bm;
	bm bulk_mutex;

	bulk & void bulka_add(std::size_t bulk){
		std::lock_guard<std::mutex> lock(mutya);
		bulk_mutex;
	}

	void bulka_feed(const bulk &b){
		std::lock_guard<std::mutex> lock(mutya);
	}

	void bulka_delete(bulk &b){
		std::lock_guard<std::mutex> lock(mutya);
	}
